    ...
    <>
      ...
      {selectedOption == "Profile" ? (
        <div className="bg-slate-200 px-2 min-h-screen pt-2">
          <p className="text-2xl font-semibold">Profile options</p>
          <form onSubmit={handleProfileChangesSubmit}>
            ...
            <div className="mt-5 mb-7">
              <div className="flex flex-col">
                <p className="text-xl">Favorite releases: </p>
                <button type="button" onClick={handleFavoriteReleasesButton} className="bg-blue-400 py-1 font-semibold border-2 border-black text-xl">Show</button>
              </div>
              {showFavoriteReleases ? (
                <div className="bg-blue-300 pl-4 flex flex-col gap-y-2 py-2 rounded-b-xl">
                  <select type="text" className="w-[80%] fav1" value={favoriteReleases.fav1} onChange={(e) => handleFavChange(e)}>
                    {releases ? releases.map((r, i) => {
                      return <option value={r.title} key={i} >{r.title}</option>
                    }): ''}
                  </select>

                  <select type="text" className="w-[80%] fav2" value={favoriteReleases.fav2} onChange={(e) => handleFavChange(e)}>
                    {releases ? releases.map((r, i) => {
                      return <option value={r.title} key={i} >{r.title}</option>
                    }): ''}
                  </select>

                  <select type="text" className="w-[80%] fav3" value={favoriteReleases.fav3} onChange={(e) => handleFavChange(e)}>
                    {releases ? releases.map((r, i) => {
                      return <option value={r.title} key={i} >{r.title}</option>
                    }): ''}
                  </select>

                  <select type="text" className="w-[80%] fav4" value={favoriteReleases.fav4} onChange={(e) => handleFavChange(e)}>
                    {releases ? releases.map((r, i) => {
                      return <option value={r.title} key={i} >{r.title}</option>
                    }): ''}
                  </select>
                  ...
                  {/* <button>Save favorite releases</button> */}
            ...
            <button type="submit" className="bg-green-400 rounded-md px-2 py-1 my-5 font-semibold border-2 border-black text-lg">
              Save changes
            </button>
          </form>
        </div>
      ...
      {selectedOption == "Auth" ? (
        <div className="min-h-screen bg-slate-200 pt-2">
          <form onSubmit={changeUserPic}>
            <p className="text-2xl font-semibold">Auth options</p>
            {/* <label htmlFor="UserImage">User image: </label>
            <img
              src={user.img}
              alt="user image"
